<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Perubahan Data User</title>
    <link rel="stylesheet" type="text/css" href="../../fontawesome/css/all.css">
	<link rel="stylesheet" type="text/css" href="../../css/bootstrap.css">
	<link rel="stylesheet" type="text/css" href="../../css/style.css">
    <link rel="icon" href="../../img/logo_gage.png" type="../../image/png">
</head>
<body id=makalah>
<h1 align="center">Perubahan Data Pengguna</h1>
<form action="" method="POST" class="container col-md-5">
        <div class="form-group">
              <label for="my-input" style="color : red;"><strong>Nama</strong></label>
              <input id="my-input" class="form-control" type="text" name="nama">
        </div>
        <div class="form-group">
              <label for="my-input" style="color : red;"><strong>Username Baru</strong></label>
              <input id="my-input" class="form-control" type="text" name="username">
        </div>
        <div class="form-group">
              <label for="my-input" style="color : red;"><strong>Password Baru</strong></label>
              <input id="my-input" class="form-control" type="text" name="password">
        </div>
        <button type="submit" name="input" class="btn btn-primary">Simpan</button>
    </form>
    
    <?php
        if (isset($_POST['input'])){
        $nama=$_POST['nama'];
        $username=$_POST['username'];
        $password=$_POST['password'];
        require "../../koneksi.php";
        $sql = "UPDATE user SET nama='$nama', username='$username', password='$password' WHERE id='$_GET[id]'";
        if (mysqli_query($conn, $sql)){
            echo "Ubah Berhasil";
            header("location:data_user.php");
            } else {
            echo mysqli_error($conn);
            }
        echo "<div class='alert alert-info'>Data Berhasil Diubah</div>";
        //echo "<meta http-equiv='refresh' content='1;url=data_user.php'>";
    }
    ?>
</body>
</html>